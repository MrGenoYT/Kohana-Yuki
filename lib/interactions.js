...
async function showPersonaModal(interaction) {
    ...
    modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('moodInput').setLabel("My Moods (max 10, 100 words / 700 chars)").setStyle(TextInputStyle.Paragraph).setValue(settings.mood).setMaxLength(700)), // Shortened label
    );
