...
async function handleInteraction(interaction, client) {
    try {
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            switch (value) {
                case 'edit_persona':
                    // Defer the update to edit the original reply
                    await interaction.deferUpdate(); 
                    const personaDashboardEmbed = new EmbedBuilder()
                        .setTitle('Yuki\'s Dashboard! 💖')
                        .setColor(0xFFB6C1)
                        .setDescription('Here you can change my core persona and behavior settings, babe!');
                    
                    const changePersonaButtonRow = new ActionRowBuilder()
                        .addComponents(
                            new ButtonBuilder()
                                .setCustomId('show_persona_modal_button') // New custom ID for the button
                                .setLabel('Change Persona and Behavior')
                                .setStyle(ButtonStyle.Primary)
                                .setEmoji('✍️') // A suitable emoji
                        );
                    
                    // Edit the original reply to show the new dashboard and button
            ...
            switch (mainId) {
                case 'toggle':
                    ...
                    break;
                case 'show_persona_modal_button': // Handle the new button click
    ...
    modal.addComponents(
        new ActionRowBuilder().addComponents(
            new TextInputBuilder()
                ...
                .setValue(settings.name.substring(0, 10)) // Ensure initial value is truncated to 10 chars
                .setMaxLength(10)
        ),
