        ...
        if (interaction.isStringSelectMenu()) {
            ...
            switch (mainId) {
                case 'personaModal':
                    ...
                    if (behavior.split(/\s+/).length > 1000) {
                        return interaction.followUp({ content: 'Behavior cannot exceed 1000 words, babe!', ephemeral: true });
                    }
                    const moodsArray = mood.split(',').map(m => m.trim()).filter(m => m.length > 0);
                    if (moodsArray.length > 10 || mood.split(/\s+/).length > 100) {
                        return interaction.followUp({ content: 'Moods cannot exceed 10 entries or 100 words total, babe!', ephemeral: true });
                    }
...
async function showPersonaModal(interaction) {
    ...
    modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('behaviorInput').setLabel("My Behavior (max 1000 words)").setStyle(TextInputStyle.Paragraph).setValue(settings.behavior).setMaxLength(7000)), // Approximate 7 chars per word
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('moodInput').setLabel("My Moods (max 10, 100 words total, comma-separated)").setStyle(TextInputStyle.Paragraph).setValue(settings.mood).setMaxLength(700)),
    );
