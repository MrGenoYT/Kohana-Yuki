...
async function handleInteraction(interaction, client) {
    try {
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            switch (value) {
                case 'edit_persona':
                    await interaction.deferUpdate(); // Defer the original interaction first
                    await interaction.deleteReply().catch(console.error); // Delete the original message

                    const dashboardEmbed = new EmbedBuilder()
                        .setTitle('Yuki\'s Dashboard! 💖')
                        .setColor(0xFFB6C1)
                        .setDescription('From here, you can change my persona and behavior, babe!');

                    const personaEditButtonRow = new ActionRowBuilder()
                        .addComponents(
                            new ButtonBuilder()
                                .setCustomId('change_persona_dialog_button') // New custom ID for this button
                                .setLabel('Change Persona and Behavior')
                                .setStyle(ButtonStyle.Primary)
                                .setEmoji('📝')
                        );
                    
                    // Send a new message with the new button
                    await interaction.followUp({ embeds: [dashboardEmbed], components: [personaEditButtonRow], ephemeral: true });
                    break;
            ...
            switch (mainId) {
                case 'toggle':
                    ...
                    break;
                case 'change_persona_dialog_button': // New handler for the dashboard button
                    // No deferUpdate() needed here, showModal() implicitly defers/responds
            ...
            switch (mainId) {
                case 'personaModal':
                    ...
                    if (behavior.split(/\s+/).length > 1000 || behavior.length > 4000) {
                    ...
                    if (moodsArray.length > 10 || mood.split(/\s+/).length > 100 || mood.length > 700) {
    ...
    modal.addComponents(
        new ActionRowBuilder().addComponents(
            new TextInputBuilder()
                ...
                .setLabel("My Name (max 10 chars)")
                .setStyle(TextInputStyle.Short)
                .setValue(settings.name.substring(0, 10))
                .setMaxLength(10)
        ),
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('behaviorInput').setLabel("My Behavior (max 1000 words / 4000 chars)").setStyle(TextInputStyle.Paragraph).setValue(settings.behavior).setMaxLength(4000)),
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('moodInput').setLabel("My Moods (max 10, 100 words / 700 chars)").setStyle(TextInputStyle.Paragraph).setValue(settings.mood).setMaxLength(700)),
    );
