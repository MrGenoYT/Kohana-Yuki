...
class KohanaYuki {
   ...
   async getUserData(userId) {
       ...
       return userData || { userId, mood: 'neutral', customInstructions: '', chatHistory: [], consecutiveNoReplies: 0, lastMessageTimestamp: 0 };
   }
   ...
   async toggleFeature(interaction, feature) {
       ...
       await interaction.followup.send({ content: `${feature === 'imageGeneration' ? 'Image Generation' : 'Web Search'} has been ${status}. uwu! ✨`, ephemeral: true });
       await this.showServerSettings(interaction); // Refresh the main settings view
   }

   async showChannelManagementOptions(interaction) {
       const settings = await this.getServerSettings(interaction.guildId);
       const currentChannels = settings.allowedChannels.map(id => `<#${id}>`).join(', ') || 'All channels allowed';
   ...
   async handleMessage(message) {
       ...
       if (!shouldReply) {
           userData.consecutiveNoReplies = (userData.consecutiveNoReplies || 0) + 1;
           await this.updateUserData(message.author.id, { consecutiveNoReplies: userData.consecutiveNoReplies });
           return;
       } else {
           userData.consecutiveNoReplies = 0;
           await this.updateUserData(message.author.id, { consecutiveNoReplies: 0 });
       }
   ...
   async determineIfShouldReply(message, settings, userData) {
       ...
       // Always reply if mentioned or replied to or contains bot name
       if (botMention || isReply || containsBotName) {
           return true;
       }

       // Reply to greetings if it's been more than 5 minutes
       ...
       // Ignore only after 2 consecutive 'NO's from the model, and if user is not typing
       if (userData.consecutiveNoReplies >= 2) {
           try {
               // This part requires checking if user is typing, which Discord.js doesn't directly expose easily for other users.
               // We'll approximate by checking if there's very recent activity from the user.
               const recentActivity = await message.channel.messages.fetch({ limit: 10 }).then(msgs =>
                   msgs.some(m => m.author.id === message.author.id && (currentTime - m.createdTimestamp) < 10000) // within last 10 seconds
               );

               if (recentActivity) {
                   return false; // User is still active, maybe completing thought
               }
           } catch (err) {
               console.error('Error fetching messages for recent activity check:', err);
           }
           // If not active, then we can reply even after 2 NOs (to avoid endless silence)
           return true;
       }
...
You should occasionally call him by his username, like "yuta," to show you remember him.
...
`;
           ...
           let chatHistory = JSON.parse(JSON.stringify(userData.chatHistory)); // Deep copy to avoid modifying original
           ...
           if (message.attachments.size > 0) {
               ...
               if (imageAttachment.contentType && imageAttachment.contentType.startsWith('image/')) {
                   try {
                       const imageBuffer = await (await fetch(imageAttachment.url)).buffer();
                       ...
                       // Use the default model as gemini-2.5-flash should handle vision
                       modelName = 'gemini-2.5-flash'; 
                   } catch (fetchError) {
                       console.error('Error fetching image for vision:', fetchError);
                       // Fallback to text-only if image fetching fails
                       await message.reply('Oopsie! I tried to see your image, babe, but something went wrong! 🥺 Can you tell me about it instead?');
           ...
           if (settings.webSearch) {
               ...
               if (searchNeeded) {
                   try {
                       ...
                       systemPrompt += '\n\nOopsie! I tried to search for you, babe, but my internet got shy! 🥺\n\n';
                   }
               }
           }
           ...
           const payload = { contents: chatHistory }; // Pass chat history as contents
           // Add the system prompt as an initial system message or prepend to the first user message if model needs it.
           // For gemini-2.5-flash, the system instruction is a separate parameter.
           // Adjusting to fit the direct fetch call provided.
           // For simple usage, adding system prompt to the first user turn or prepending to chat history
           
           payload.contents = [{ role: 'user', parts: [{ text: systemPrompt + '\n' + message.content }] }];
           if (chatHistory.length > 0) {
                payload.contents = chatHistory; // Keep actual chat history for context
                payload.contents[0].parts[0].text = systemPrompt + '\n' + payload.contents[0].parts[0].text; // Prepend system prompt to the first message for context
           } else {
                payload.contents = [{ role: 'user', parts: [{ text: systemPrompt + '\n' + message.content }] }];
           }

           // Ensure the user's current message is at the end of the history if not already.
           // This is handled by `addMessageToMemory` and `chatHistory` construction.
           ...
           await message.reply('Ahhh! Something went wrong while I was thinking, babe! My brain went uwu! 😵‍💫');
       }
   }

   async performWebSearch(query) {
       // Dummy implementation for search. In a real scenario, integrate with a search API.
       // For the purpose of this exercise, we'll simulate a search result.
       // If a real search API is used, ensure it's allowed by Google's terms of service.

       const apiKey = process.env.GEMINI_API_KEY || ""; // Using Gemini API key, assuming it's allowed for search calls within this context
